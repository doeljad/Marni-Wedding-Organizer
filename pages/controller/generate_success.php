...
function generateInvoicePDF($orderId)
{
    ...
    $sql = "SELECT t.*, p.*, pl.nama_paket, pl.harga AS harga_paket, pel.nama
            FROM transaksi t
    ...
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(40, 10, 'Harga Paket:', 0, 0);
    $pdf->SetFont('Arial', '', 12);
    $pdf->Cell(0, 10, number_format($result['harga_paket'], 0, ',', '.'), 0, 1);
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(40, 10, 'Total Bayar:', 0, 0);
    $pdf->SetFont('Arial', '', 12);
    $pdf->Cell(0, 10, number_format($result['total'], 0, ',', '.'), 0, 1);

    // Menentukan kekurangan jika ada
    if ($result['total'] < $result['harga_paket']) {
        $kekurangan = $result['harga_paket'] - $result['total'];
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(40, 10, 'Kekurangan:', 0, 0);
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(0, 10, number_format($kekurangan, 0, ',', '.'), 0, 1);
    }
